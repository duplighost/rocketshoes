...
import { ISO, applyIsoZ, billboardTransform, screenDirFromWorld } from './camera.js';
...
export function drawPlayer(ctx, p, room) {
  ...
  const pose = { speed: sp, vx: p.vx || 0, vy: p.vy || 0, moveFace: p.moveFace || 0, animT: p.animT || 0, dash: p.dashT > 0, rail: !!p.rail?.active, vent: p.ventT > 0, surface: p._surface, aim: p.face || 0, bodyFace, flip: p._bodyFlip || 1, level: p.level || 0, airZ: p.airZ || 0 };
  ...
  for (let i = p.after.length - 1; i >= 0; i--) {
    ...
    drawPlayerBody(ctx, a.x, a.y, a.face, pal, aAlpha, true, a.spin || 0,
      { speed: a.dash ? 520 : 120, moveFace: a.moveFace || a.face, animT: a.animT || 0, rail: a.rail, level: a.level ?? (p.level || 0), airZ: a.airZ || 0 });
  }
  if (p.dashT > 0) {
...
function drawRocketBoots(ctx, p, pose, pal) {
  ...
  let dx, dy;
  if (sp > 24) {
    const sd = screenDirFromWorld(p.vx || 0, p.vy || 0);
    dx = -sd.x; dy = -sd.y;                    // thrust opposes projected travel
  } else { dx = 0; dy = 1; }                   // hover: jets drop straight down the screen
  ...
  const px = -dy, py = dx;
  const bootY = 17 * PLAYER_EFFECT_SCALE;
  const z = (p.level || 0) * ISO.TIER_Z + (p.airZ || 0);
  ctx.save();
  billboardTransform(ctx, p.x, p.y, z);
  ctx.globalCompositeOperation = 'lighter';
  for (const side of [-1, 1]) {
    const ox = side * 8.5 * PLAYER_EFFECT_SCALE;
    const oy = bootY;
...
export function drawPlayerBody(ctx, x, y, face, pal, alpha = 1, ghost = false, spinPhase = 0, pose = {}) {
  ...
  const z = ((pose.level || 0) * ISO.TIER_Z) + (pose.airZ || 0);
  billboardTransform(ctx, x, y, z);
...
function drawEnemyFloorTelegraph(ctx, e, room, pal) {
  const z = (e.level || 0) * ISO.TIER_Z;
  if (e.type === 'charger' && e.state === 'windup') {
    const dx = e.chargeX || e.vx || 1, dy = e.chargeY || e.vy || 0;
    const m = Math.hypot(dx, dy) || 1;
    ctx.save();
    if (z) applyIsoZ(ctx, z);
    ctx.globalCompositeOperation = 'lighter';
    ctx.globalAlpha = 0.38 + 0.30 * Math.sin(e.phase * 30);
    ctx.strokeStyle = pal.bad; ctx.lineWidth = 3; ctx.setLineDash([10, 8]);
    ctx.beginPath(); ctx.moveTo(e.x + dx / m * e.r, e.y + dy / m * e.r); ctx.lineTo(e.x + dx / m * 390, e.y + dy / m * 390); ctx.stroke();
    ctx.setLineDash([]); ctx.restore();
  }
  if (e.type === 'sniper' && e.aimT > 0 && e.snipeX !== undefined) {
    const dx = e.snipeX || 1, dy = e.snipeY || 0;
    const m = Math.hypot(dx, dy) || 1;
    ctx.save();
    if (z) applyIsoZ(ctx, z);
    ctx.globalCompositeOperation = 'lighter';
    ctx.globalAlpha = 0.35; ctx.strokeStyle = pal.bad; ctx.lineWidth = 4;
    ctx.beginPath(); ctx.moveTo(e.x, e.y); ctx.lineTo(e.x + dx / m * 1020, e.y + dy / m * 1020); ctx.stroke();
    ctx.restore();
  }
}
...
export function drawEnemy(ctx, e, room) {
  ...
  drawEnemyFloorTelegraph(ctx, e, room, pal);
  ctx.save();
  billboardTransform(ctx, e.x, e.y, (e.level || 0) * ISO.TIER_Z);
  ...
  const ez = (e.level || 0) * ISO.TIER_Z;

  // Warden's rotating shield: billboarded armor arc with one open GAP. In iso the
  // opening stays legible instead of flattening into a floor noodle.
  if (e.shield) {
    const sr = e.r + 16, gap = e.gapHalf || 0.6, spark = (e.shieldSpark || 0) > 0;
    ctx.save();
    billboardTransform(ctx, e.x, e.y, ez);
    ctx.lineCap = 'round';
    ctx.strokeStyle = spark ? '#ffffff' : hexA('#ffe27d', 0.7);
    ctx.shadowColor = '#ffe27d'; ctx.shadowBlur = spark ? 16 : 8;
    ctx.lineWidth = 6;
    ctx.beginPath(); ctx.arc(0, 0, sr, e.shieldAngle + gap, e.shieldAngle + TAU - gap); ctx.stroke();
    ctx.globalAlpha = 0.8; ctx.strokeStyle = hexA('#ffffff', 0.85); ctx.lineWidth = 2.4; ctx.shadowBlur = 0;
    const ga = e.shieldAngle;
    ctx.beginPath(); ctx.arc(Math.cos(ga) * sr, Math.sin(ga) * sr, 7, 0, TAU); ctx.stroke();
    ctx.restore();
  }

  // staggered: reeling from a dash blow — woozy stun-stars orbit above the head.
  if (!e.boss && e.stun > 0.12) {
    ...
    for (let i = 0; i < 3; i++) {
      const a = t * 7 + (i / 3) * TAU;
      starPath(ctx, Math.cos(a) * e.r * 0.8, -e.r - 7 + Math.sin(a) * 3.5, 3.4, 1.5, 5);
      ctx.fill();
    }
    ctx.restore();
  }
  ...
  if (e.captain) {
    ...
    ctx.beginPath();
    ctx.moveTo(0, ty - 11); ctx.lineTo(5, ty - 6); ctx.lineTo(0, ty - 1); ctx.lineTo(-5, ty - 6);
    ctx.closePath(); ctx.fill();
    ctx.globalAlpha = 0.8;
    ctx.font = '800 9px Inter, system-ui, sans-serif';
    ctx.fillText(e.captain.toUpperCase(), 0, ty + 11);
    ctx.restore();
  }
  if (e.maxHp > 6 && e.hp < e.maxHp) {
    ...
    ctx.fillRect(-w / 2, -e.r - 12, w * clamp(e.hp / e.maxHp, 0, 1), 4);
    ctx.restore();
  }
