...
import { view, ISO, cam, beginCameraFrame, applyWorldTransform, applyIsoZ, uiTransform, worldToScreen, visibleWorldRect } from './camera.js';
...
const TIER_LIFT = ISO.TIER_Z; // screen-up px a platform rises in the isometric projection
...
export function drawFrame() {
  ...
  // The arena floats inside a vast neon metropolis: a night-sky gradient + a procedural
  // isometric skyline fill the void that used to be dead black. Both render behind the
  // floor so gameplay always reads on top.
  drawSkyBackdrop(pal);
  drawExteriorCity(room, pal);
  ...
  ctx.globalAlpha = 1;
  ctx.fillStyle = 'rgba(5,7,13,0.24)';
  ctx.fillRect(0, 0, room.w, room.h);
  drawIsoGrid(room, pal);        // diamond city grid: the new isometric read
  ...
  drawShop(room, pal, p);
  if (room.flipPad) drawFlipPad(room, pal);
  ...
  for (const o of room.obstacles) if (!o.gone) {
    ...
    renderables.push({ depth: o.type === 'circle' ? o.x + o.y + o.rad : o.x + o.y + o.w + o.h, lv, lift: (o.level || 0) * LIFT, draw: () => drawObstacle(ctx, o, room) });
  }
  for (const e of room.enemies) renderables.push({ depth: e.x + e.y + e.r, lv: e.level || 0, lift: (e.level || 0) * LIFT, draw: () => drawEnemy(ctx, e, room) });
  if (p && !p.dead) renderables.push({ depth: p.x + p.y + p.r + 6, lv: p.level || 0, lift: (p.level || 0) * LIFT + (p.airZ || 0), draw: () => drawPlayer(ctx, p, room) });
  // Depth order flips with the view: when cam.flip is -1 the far corner becomes the near
  // one, so multiply the depth key by the flip sign. Height (lv) still sorts low→high.
  renderables.sort((a, b) => (a.lv - b.lv) || (a.depth - b.depth) * cam.flip);
  for (const r of renderables) {
    if (r.lift) { ctx.save(); applyIsoZ(ctx, r.lift); r.draw(); ctx.restore(); }
    else r.draw();
  }
...
function drawTiers(room, pal) {
  ...
  for (const t of room.tiers) {
    const z = TIER_LIFT;
    const top = [
      worldToScreen(t.x, t.y, z),
      worldToScreen(t.x + t.w, t.y, z),
      worldToScreen(t.x + t.w, t.y + t.h, z),
      worldToScreen(t.x, t.y + t.h, z),
    ];
    const base = [
      worldToScreen(t.x, t.y, 0),
      worldToScreen(t.x + t.w, t.y, 0),
      worldToScreen(t.x + t.w, t.y + t.h, 0),
      worldToScreen(t.x, t.y + t.h, 0),
    ];
    ...
    const faces = [
      { pts: [top[1], top[2], base[2], base[1]], fill: mixHex(pal.bg, '#000000', 0.18), a: 0.78 },
      { pts: [top[2], top[3], base[3], base[2]], fill: mixHex(pal.bg, '#000000', 0.34), a: 0.86 },
      { pts: [top[0], top[3], base[3], base[0]], fill: mixHex(pal.floor, '#000000', 0.42), a: 0.48 },
    ];
    ...
    ctx.strokeStyle = hexA(pal.accent, 0.42); ctx.lineWidth = 1.4;
    for (let i = 0; i < 5; i++) {
      const a = i / 5;
      const p0 = lerp2(top[2], top[3], a), p1 = lerp2(base[2], base[3], a);
      ctx.beginPath(); ctx.moveTo(p0.x, p0.y); ctx.lineTo(p1.x, p1.y); ctx.stroke();
    }

    // walkable roof tile: brighter top plane with neon rim and subtle inset grid.
    ctx.globalAlpha = 1;
    const g = ctx.createLinearGradient(0, Math.min(...top.map(p => p.y)), 0, Math.max(...top.map(p => p.y)) + 1);
    g.addColorStop(0, mixHex(pal.floor, pal.accent, 0.30));
    g.addColorStop(1, mixHex(pal.floor, '#ffffff', 0.07));
    ctx.fillStyle = g;
    poly(top); ctx.fill();
    ctx.globalCompositeOperation = 'lighter';
    ctx.strokeStyle = hexA(pal.accent2, 0.78); ctx.lineWidth = 2.2;
    poly(top); ctx.stroke();
    ctx.strokeStyle = hexA('#ffffff', 0.23); ctx.lineWidth = 1;
    const cols = Math.max(2, Math.floor(t.w / 190));
    const rows = Math.max(2, Math.floor(t.h / 170));
    for (let i = 1; i < cols; i++) {
      const a = i / cols;
      const p0 = worldToScreen(t.x + t.w * a, t.y + 12, z + 1);
      const p1 = worldToScreen(t.x + t.w * a, t.y + t.h - 12, z + 1);
      ctx.globalAlpha = 0.16; ctx.beginPath(); ctx.moveTo(p0.x, p0.y); ctx.lineTo(p1.x, p1.y); ctx.stroke();
    }
    for (let i = 1; i < rows; i++) {
      const a = i / rows;
      const p0 = worldToScreen(t.x + 12, t.y + t.h * a, z + 1);
      const p1 = worldToScreen(t.x + t.w - 12, t.y + t.h * a, z + 1);
      ctx.globalAlpha = 0.14; ctx.beginPath(); ctx.moveTo(p0.x, p0.y); ctx.lineTo(p1.x, p1.y); ctx.stroke();
    }

    // Ramp/stair strip: a bright playable seam instead of a hidden top-down rectangle.
    if (t.ramp) {
      const sw = Math.min(t.ramp.w || 150, t.w * 0.55);
      const x0 = t.ramp.x - sw / 2, x1 = t.ramp.x + sw / 2;
      ctx.globalAlpha = 0.72 + Math.sin(tNow * 5) * 0.08;
      ctx.strokeStyle = hexA('#ffffff', 0.72); ctx.lineWidth = 2.6;
      for (let i = 0; i < 4; i++) {
        const yy = t.y + t.h - (i + 0.5) * 16;
        const a = i / 4;
        const p0 = worldToScreen(x0 + sw * a * 0.16, yy, z * (i + 1) / 4);
        const p1 = worldToScreen(x1 - sw * a * 0.16, yy, z * (i + 1) / 4);
        ctx.beginPath(); ctx.moveTo(p0.x, p0.y); ctx.lineTo(p1.x, p1.y); ctx.stroke();
      }
    }
    ctx.restore();
...
function drawVents(room, pal, p) {
  ...
  for (const v of vents) {
    ...
    ctx.save(); ctx.translate(v.x, v.y); applyIsoZ(ctx, lift);
    ctx.globalCompositeOperation = 'lighter';
...
function visibleRect(margin = 0) {
  return visibleWorldRect(margin);
}


function drawIsoGrid(room, pal) {
  const vis = visibleRect(220);
  const step = 160;
  const x0 = Math.max(0, Math.floor(vis.l / step) * step), x1 = Math.min(room.w, Math.ceil(vis.r / step) * step);
  const y0 = Math.max(0, Math.floor(vis.t / step) * step), y1 = Math.min(room.h, Math.ceil(vis.b / step) * step);
  const t = room.time || performance.now() / 1000;
  ctx.save();
  ctx.lineCap = 'round';
  ctx.globalCompositeOperation = 'lighter';
  ...
  for (let x = x0; x <= x1; x += step) {
    ctx.globalAlpha = (x % (step * 2) === 0 ? 0.16 : 0.085);
    ctx.strokeStyle = x % (step * 4) === 0 ? hexA(pal.accent2, 0.45) : hexA(pal.accent, 0.34);
    ctx.beginPath(); ctx.moveTo(x, y0); ctx.lineTo(x, y1); ctx.stroke();
  }
  for (let y = y0; y <= y1; y += step) {
    ctx.globalAlpha = (y % (step * 2) === 0 ? 0.15 : 0.08);
    ctx.strokeStyle = y % (step * 4) === 0 ? hexA(pal.accent3, 0.42) : hexA(pal.accent, 0.30);
    ctx.beginPath(); ctx.moveTo(x0, y); ctx.lineTo(x1, y); ctx.stroke();
  }
  // Fast parallax pulse through the diamonds. It makes the floor feel alive without
  // creating new collision or burning per-room assets.
  ctx.globalAlpha = 0.10;
  ctx.strokeStyle = hexA('#ffffff', 0.38);
  ctx.lineWidth = 2;
  const sweep = ((t * 520) % (step * 2)) - step;
  for (let y = y0 - step * 2; y <= y1 + step * 2; y += step * 2) {
    ctx.beginPath(); ctx.moveTo(x0, y + sweep); ctx.lineTo(x1, y + sweep); ctx.stroke();
  }
  ctx.restore();
}
...
function drawPickups(room, pal) {
  const t = performance.now() / 1000;
  const revealed = cam.flip < 0;
  for (const q of room.pickups) {
    const bob = Math.sin(t * 4 + q.x * 0.01) * 3;
    ctx.save();
    ctx.translate(q.x, q.y + bob - (q.level || 0) * TIER_LIFT);
    // Flip-hidden caches show only a faint shimmer until you flip the view to reveal them.
    if (q.flipHidden) ctx.globalAlpha = revealed ? 1 : 0.16;
    if (q.type === 'spark') {
...
// The view-flip button: a pulsing isometric floor disc with a ⟳ glyph and a soft beacon,
// readable from across the giant map. Stepping on it (player.js) flips the camera 180°.
function drawFlipPad(room, pal) {
  const pad = room.flipPad;
  const t = room.time || performance.now() / 1000;
  const c = worldToScreen(pad.x, pad.y, 0);
  const rx = pad.r * ISO.X * view.scale, ry = pad.r * ISO.Y * view.scale;
  const pulse = 0.6 + 0.4 * Math.sin(t * 4 + (pad.pulse || 0));
  uiTransform(ctx);
  ctx.save();
  ctx.globalCompositeOperation = 'lighter';
  ctx.shadowColor = pal.accent2; ctx.shadowBlur = 18;
  // filled disc
  ctx.globalAlpha = 0.16 + 0.12 * pulse; ctx.fillStyle = pal.accent2;
  ctx.beginPath(); ctx.ellipse(c.x, c.y, rx, ry, 0, 0, TAU); ctx.fill();
  // pulsing rim
  ctx.globalAlpha = 0.5 + 0.4 * pulse; ctx.lineWidth = 3; ctx.strokeStyle = pal.accent2;
...
// Night-sky backdrop: a soft vertical gradient (deep indigo overhead → faint biome glow
// down low) so the void around the arena reads as sky, not a dead black rectangle.
function drawSkyBackdrop(pal) {
  uiTransform(ctx);
  const g = ctx.createLinearGradient(0, 0, 0, view.H);
  g.addColorStop(0, mixHex(pal.bg, '#01010a', 0.55));
  g.addColorStop(0.6, mixHex(pal.bg, '#03040c', 0.32));
  g.addColorStop(1, mixHex(pal.bg, pal.accent, 0.12));
  ctx.fillStyle = g;
  ctx.fillRect(0, 0, view.W, view.H);
}

// The surrounding metropolis. Each building is an extruded iso box (two visible faces +
...
// Player shots are now LASER BOLTS, not specks: a long neon streak with a hot white
// core and a bright head, lifted off the floor so they read as fired energy. Enemy
// shots stay as glowing orbs so the two never read the same. Drawn in iso floor space,
// so a bolt points along its real travel direction in the projection.
const LASER_LIFT = 15;
function drawBullets(room) {
  for (const b of room.bullets) {
    ...
    if (b.owner === 'player' && !b.converted) {
      ...
      applyIsoZ(ctx, (b.level || 0) * TIER_LIFT + LASER_LIFT);
      const len = b.r * (b.primed ? 4.6 : 3.6) + Math.min(46, sp * 0.052);
      const tailX = b.x - ux * len, tailY = b.y - uy * len;
      const headX = b.x + ux * b.r * 0.6, headY = b.y + uy * b.r * 0.6;
      ctx.lineCap = 'round';
      ...
      ctx.beginPath(); ctx.moveTo(b.x - ux * len * 0.6, b.y - uy * len * 0.6); ctx.lineTo(headX, headY); ctx.stroke();
      // bright muzzle-tip head
      ctx.fillStyle = '#ffffff';
      ctx.beginPath(); ctx.arc(headX, headY, b.r * 0.7, 0, TAU); ctx.fill();
      if (b.primed) { // empowered shot: extra colored halo at the head
        ctx.globalAlpha = 0.8; ctx.fillStyle = b.color;
        ctx.beginPath(); ctx.arc(headX, headY, b.r * 1.25, 0, TAU); ctx.fill();
      }
      ctx.restore();
      continue;
    }
    ...
    applyIsoZ(ctx, (b.level || 0) * TIER_LIFT);
...
function drawDangerTriangles(room, p) {
  ...
  for (const e of room.enemies) {
    if (e.hp <= 0) continue;
    const pos = worldToScreen(e.x, e.y, (e.level || 0) * TIER_LIFT);
...
function drawPortalArrow(room) {
  const po = room.portal;
  const pos = worldToScreen(po.x, po.y, 0);
  const sx = pos.x, sy = pos.y;
  const margin = 46;
...
function drawEclipse(room) {
  ...
  const mp = worldToScreen(moon.x, moon.y, (moon.level || 0) * TIER_LIFT);
...
function drawSpeedStreaks(p) {
  ...
  const z = (p.level || 0) * TIER_LIFT + (p.airZ || 0);
  const pos = worldToScreen(p.x, p.y, z);
  const ahead = worldToScreen(p.x + p.vx * 0.08, p.y + p.vy * 0.08, z);
  const px = pos.x, py = pos.y;
  const ang = Math.atan2(ahead.y - pos.y, ahead.x - pos.x) + Math.PI;     // behind the direction of travel
  ctx.save();
  ctx.globalCompositeOperation = 'lighter';
