...
export function updatePickups(room, dt) {
  ...
  const revealed = cam.flip < 0;   // the flipped view reveals hidden caches
  for (let i = room.pickups.length - 1; i >= 0; i--) {
    const q = room.pickups[i];
    // Flip-hidden caches lie dormant until you flip the view: no decay, no magnet, no
    // pickup. Once revealed they behave like any other pickup.
    if (q.flipHidden && !revealed) continue;
    q.life -= dt;
