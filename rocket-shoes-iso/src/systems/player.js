...
// Flip the camera 180° with a satisfying punctuation. Shared by the floor button and the
// keyboard shortcut so both feel identical.
export function flipView(room) {
  toggleViewFlip();
  addFlash(0.55); addShake(0.32); slowMo(0.05);
  sfx('portal'); haptic(26);
  const p = state.run?.player;
  if (room && p) ripple(room, p.x, p.y, room.biome.pal.accent2, 130, 0.36);
}

// Big floor button: step on it to flip the view and see the other side of the city (and
// arm the caches hidden from the default angle). Debounced so you don't oscillate.
function checkFlipPad(p, room, dt) {
  const pad = room?.flipPad;
  if (!pad) return;
  pad.cd = Math.max(0, (pad.cd || 0) - dt);
  if (pad.cd > 0) return;
  if (dist(p.x, p.y, pad.x, pad.y) < pad.r + p.r) {
    pad.cd = 1.4;
    flipView(room);
    ripple(room, pad.x, pad.y, room.biome.pal.accent2, 150, 0.4);
  }
}

export function updatePlayer(p, move, aim, room, dt) {
  checkFlipPad(p, room, dt);
...
function applyFlowLanes(p, room, move, dt) {
  ...
  if (!move.active && !ridingDash) return false;
  const dirX = ridingDash ? p.vx : move.x;
  const dirY = ridingDash ? p.vy : move.y;
  ...
  for (const l of lanes) {
    ...
    if (!ridingDash && absAlong < 0.25) continue;
    if (ridingDash && absAlong < 0.14) continue;
  ...
  const intentStrength = ridingDash ? 1 : clamp((Math.abs(bestAlong) - 0.25) / 0.75, 0, 1);
  const boost = (l.boost || 210) * (0.32 + falloff * 0.74) * (0.35 + intentStrength * 0.65) * (ridingDash ? 1.55 : 1);
  ...
  const steer = (0.025 + falloff * 0.07) * (0.35 + intentStrength * 0.65) * (ridingDash ? 0.55 : 1);
  p.vx -= s.nx * lateral * steer;
  p.vy -= s.ny * lateral * steer;
  p.flowT = 0.16;
  if (Math.random() < (ridingDash ? 0.42 : 0.14 + intentStrength * 0.08)) {
...
function finishPlayerFrame(p, room, sp, dt, movingIntent) {
  ...
  p.after.unshift({
    ...
    dash: dashLike, moveFace: p.moveFace, animT: p.animT, rail: !!p.rail?.active, level: p.level || 0, airZ: p.airZ || 0,
  });
...
function tryLatchSkyRail(p, room, x0, y0, startLevel = p.level || 0) {
  ...
  p.rail = { active: true, kind: 'sky', rail: best, u: bestInfo.t, dir, speed: Math.max(760, Math.abs(dot) * 0.72), rocketT: 0, t: 0 };
  p.dashT = 0;
  p.dashCd = 0;
...
function detachRail(p, room, info, ix, iy, opts = {}) {
  ...
  p.flowT = Math.max(p.flowT || 0, 0.08);
...
function updateRailRide(p, room, move, dt) {
  ...
  p.level = 0; p.stillT = 0; p.flowT = Math.max(p.flowT || 0, 0.08);
  if (p.rail.rocketT > 0) {
...
function updateSkyRailRide(p, room, move, dt) {
  ...
  p.vx = info.tx * (p.rail.dir || 1) * p.rail.speed;
  p.vy = info.ty * (p.rail.dir || 1) * p.rail.speed;
  p.level = r.level || 1; p.stillT = 0; p.flowT = Math.max(p.flowT || 0, 0.10);
  if (p.rail.rocketT > 0) {
    if (!(p._dashHitIds instanceof Set)) p._dashHitIds = new Set();
...
function performDashCut(p, room, range) {
  ...
  for (const e of room.enemies) {
    ...
    if (dist(p.x, p.y, e.x, e.y) < range + e.r) {
      ...
      // Make it obvious the dash cut through: white core, enemy-color shatter, and a crisp slash ring.
      burst(room, e.x, e.y, '#ffffff', 12, 315, 0.30, 3.6);
      burst(room, e.x, e.y, e.color || room.biome.pal.accent3, 9, 250, 0.34, 2.9);
      ripple(room, e.x, e.y, '#ffffff', 62, 0.30);
  ...
  if (hits) {
    sfx('slice');
    addFlash(0.24); addShake(0.36); slowMo(0.058);
    hitPause(hits > 1 ? 'dashKill' : 'dash');
...
// Soft dash homing: pick the best enemy in the player's dash lane and blend toward it.
// The perpendicular gate is the important part: it nudges a dash that was already
// trying to hit something; it does not rip control away toward random enemies.
function dashHomingVector(p, room, n) {
  if (!room?.enemies?.length || p.rail?.active) return n;
  const range = PLAYER.DASH_HOMING_RANGE || 660;
  const cone = PLAYER.DASH_HOMING_CONE || 0.50;
  const maxPerp = PLAYER.DASH_HOMING_PERP || 280;
  let best = null, bestScore = -Infinity;
  for (const e of room.enemies) {
    if (e.hp <= 0 || (e.level || 0) !== (p.level || 0)) continue;
    const dx = e.x - p.x, dy = e.y - p.y;
    const d = Math.hypot(dx, dy);
    if (d < 28 || d > range + (e.r || 0)) continue;
    const ux = dx / d, uy = dy / d;
    const dot = ux * n.x + uy * n.y;
    if (dot < cone) continue;
    const perp = Math.abs(dx * -n.y + dy * n.x);
    if (perp > maxPerp + (e.r || 16)) continue;
    const score = dot * 1.65 + (1 - d / range) * 0.95 - perp / (maxPerp * 3.1) + (e.boss ? 0.18 : 0);
    if (score > bestScore) { bestScore = score; best = { ux, uy, d, dot, perp }; }
  }
  if (!best) return n;
  const closeness = clamp(1 - best.d / range, 0, 1);
  const alignment = clamp((best.dot - cone) / Math.max(0.001, 1 - cone), 0, 1);
  const blend = (PLAYER.DASH_HOMING_BLEND || 0.40) * (0.32 + closeness * 0.44 + alignment * 0.30);
  const steered = norm(n.x * (1 - blend) + best.ux * blend, n.y * (1 - blend) + best.uy * blend);
  if (steered.m > 0.08) {
    if (!reduced()) particle(room, p.x + steered.x * 24, p.y + steered.y * 24, '#ffffff', steered.x * 90, steered.y * 90, 0.12, 3.2);
    return steered;
  }
  return n;
}
...
export function tryDash(dx = null, dy = null, move = null) {
  ...
  // Slight homing: if an enemy is roughly in the dash lane, curve the launch toward
  // it. This happens after rail handling so it never steals a deliberate rail dash.
  n = dashHomingVector(p, room, n);
