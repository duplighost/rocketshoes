...
export function updateBullets(room, dt) {
  ...
  for (let i = room.bullets.length - 1; i >= 0; i--) {
    ...
    b.life -= dt;
    // Shot homing. Every player bolt gets a gentle BASELINE pull toward an enemy that's
    // roughly ahead of it (forward cone) so iso aiming is forgiving; the hunterMycelia
    // relic (b.turn) stacks on top and tracks full-circle. Curving the velocity keeps
    // the bolt's laser streak pointing where it's actually going.
    if (b.owner === 'player') {
      ...
      if (turn > 0) {
        ...
        for (const e of room.enemies) {
          ...
          if (align < minAlign) continue;              // don't yank toward foes behind the bolt
          const score = d * (1.7 - align);             // favour close AND well-aligned
