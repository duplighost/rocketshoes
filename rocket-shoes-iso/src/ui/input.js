...
import { screenToWorld, screenDirToWorld } from '../render/camera.js';
import { tryDash, flipView } from '../systems/player.js';
...
function releaseTouches(list) {
  for (const t of list) {
    if (t.identifier === aimTouch.id) {
      ...
      if (!suppressed() && flick) { const d = screenDirToWorld(aimTouch.dx, aimTouch.dy); tryDash(d.x, d.y); }
      clearPad(aimTouch);
    }
    if (t.identifier === moveTouch.id) {
      const quick = now() - moveTouch.startT < 230;
      const flick = moveTouch.maxLen > 0.80 && moveTouch.speed > 560;
      if (quick && flick && !suppressed() && state.run?.player?.dashCd <= 0) { const d = screenDirToWorld(moveTouch.dx, moveTouch.dy); tryDash(d.x, d.y); }
      clearPad(moveTouch);
    }
  }
}
...
export function tickTouchDash() {
  ...
  if (moveTouch.id !== null && moveTouch.len > 0.82 && p.dashCd <= 0 && !moveTouch.dashLatch) {
    ...
    if (slammedFromRest) {
      const d = screenDirToWorld(moveTouch.dx, moveTouch.dy);
      tryDash(d.x, d.y);
  ...
  if (aimTouch.id !== null && aimTouch.len > 0.82 && p.dashCd <= 0 && !aimTouch.dashLatch) {
    ...
    if (age < 230 && aimTouch.speed > 920) {
      const d = screenDirToWorld(aimTouch.dx, aimTouch.dy);
      tryDash(d.x, d.y);
...
function onKeyDown(e) {
  ...
  if (state.mode === 'play' && !suppressed()) {
    ...
    if (k === 'q') flipView(state.room); // flip the isometric view 180° (also a floor button)
    if (k === 'r') boonReroll();
  }
...
function getMoveRaw(x, y) {
  ...
  if (l > 1) { x /= l; y /= l; }
  const w = screenDirToWorld(x, y);
  return { x: w.x, y: w.y, active: l > 0.06, l: Math.min(1, l) };
}
...
export function getAim() {
  ...
  if (gp) {
    ...
    if (rx || ry) {
      const n = screenDirToWorld(rx, ry);
